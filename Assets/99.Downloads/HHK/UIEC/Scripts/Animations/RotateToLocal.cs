...
namespace HHK.UIEC
{
    ...
    public class RotateToLocal : UIECAnimation
    {
        ...
        protected override Sequence DoAnimation(GameObject target)
        {
            if (type == MoveType.Actual)
                return DOTween.Sequence().Append(target.transform.DOLocalRotate(to, duration, mode).SetEase(ease));
            else
                return DOTween.Sequence().Append(target.transform.DOLocalRotate(target.transform.localEulerAngles + to, duration, mode).SetEase(ease));
        }
        ...
        void Mark()
        {
            ...
            foreach (var o in objs)
                to = o.transform.localEulerAngles;
        }
        void Set()
        {
            var objs = EditorFindUs();
            foreach (var o in objs)
                o.transform.localEulerAngles = to;
        }
#endif
    }
}