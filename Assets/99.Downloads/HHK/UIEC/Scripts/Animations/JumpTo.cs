...
namespace HHK.UIEC
{
    public class JumpTo : UIECAnimation
    {
        ...
        protected override Sequence DoAnimation(GameObject target)
        {
            if (type == MoveType.Actual)
                return target.transform.DOJump(to, jumpPower, numJumps, duration, snapping).SetEase(ease);
            else

                return target.transform.DOJump(target.transform.position + to, jumpPower, numJumps, duration, snapping).SetEase(ease);
