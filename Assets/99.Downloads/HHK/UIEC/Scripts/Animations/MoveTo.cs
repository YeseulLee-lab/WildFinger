...
namespace HHK.UIEC
{
    ...
    public class MoveTo : UIECAnimation
    {
        ...
        protected override Sequence DoAnimation(GameObject target)
        {
            if (type == MoveType.Actual)
                return DOTween.Sequence().Append(target.transform.DOMove(to, duration, snapping).SetEase(ease));
            else
                return DOTween.Sequence().Append(target.transform.DOMove(target.transform.position + to, duration, snapping).SetEase(ease));
        }
